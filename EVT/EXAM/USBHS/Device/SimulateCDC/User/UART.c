...
{
    RCC_HB2PeriphClockCmd( RCC_HB2Periph_AFIO | RCC_HB2Periph_GPIOC | RCC_HB2Periph_GPIOA, ENABLE );
    RCC_HB1PeriphClockCmd( RCC_HB1Periph_USART2 | RCC_HB1Periph_TIM2, ENABLE );
...
void UART_CfgInit( uint32_t baudrate, uint8_t stopbits, uint8_t parity )
{
    ...
    /* Configure USART Rx (PC13) as input floating */
    GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_13;
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_IPU;
    GPIO_Init( GPIOC, &GPIO_InitStructure );
    GPIO_PinAFConfig(GPIOC,GPIO_PinSource13,GPIO_AF1);

    /* Configure USART2 Tx (PC12) as alternate function push-pull */
    GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_12;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_High;
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_AF_PP;
    GPIO_Init( GPIOC, &GPIO_InitStructure );
    GPIO_PinAFConfig(GPIOC,GPIO_PinSource12,GPIO_AF1);
...
void UART_Init( uint8_t mode, uint32_t baudrate, uint8_t stopbits, uint8_t parity )
{
    ...
    UART_DMAInit( 0, &UART_Tx_Buf[ 0 ], DEF_UARTx_TX_BUF_LEN );

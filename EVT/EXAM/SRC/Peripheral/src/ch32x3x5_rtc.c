...
*******************************************************************************/
#include "ch32x3x5_rtc.h"

/* RTC_Private_Defines */
#define RTC_LSB_MASK     ((uint32_t)0x0000FFFF) /* RTC LSB Mask */
#define PRLH_MSB_MASK    ((uint32_t)0x000F0000) /* RTC Prescaler MSB Mask */

#define CTLR_CAL_MASK    ((uint32_t)0xFFFC07FF)
#define CTLR_MASK        ((uint32_t)0xFFFFF8FF)
 ...
 *
 * @param   RTCOutputSource - specifies the RTC output source.
 *            RTCOutputSource_None - no RTC output on the Tamper pin.
 *            RTCOutputSource_CalibClock - output the RTC clock with
 *        frequency divided by 64 on the Tamper pin.
 *            RTCOutputSource_Alarm - output the RTC Alarm pulse signal
 *        on the Tamper pin.
 *            RTCOutputSource_Second - output the RTC Second pulse
 *        signal on the Tamper pin.
 *
 * @return  none
 */
void RTC_OutputConfig(uint32_t RTCOutputSource)
{
    uint32_t tmpreg = 0;

    tmpreg = RTC->CTLRL;
    tmpreg &= (uint32_t)CTLR_MASK;
    tmpreg |= RTCOutputSource;
    RTC->CTLRL = tmpreg;
}

/*********************************************************************
 * @fn      RTC_SetRTCCalibrationValue
 *
 ...
 * @return  none
 */
void RTC_SetRTCCalibrationValue(uint8_t CalibrationValue)
{
    uint32_t tmpreg = 0;

    tmpreg = RTC->CTLRL;
    tmpreg &= CTLR_CAL_MASK;
    tmpreg |= CalibrationValue << 11;
    RTC->CTLRL = tmpreg;
}
 ...
 * @fn      RTC_RTCCLKConfig
 *
 * @brief    the RTC clock selecte .
 *
 * @param   RTC_RTCCLKSource - specifies the RTC clock source.
 *            RTC_RTCCLKSource_LSI - LSI selected as RTC clock.
 *            RTC_RTCCLKSource_HSE_Div512 - HSE clock divided by 512 selected as RTC clock.
 *         Note-   
 *           Once the RTC clock is selected it can't be changed unless the Backup domain is reset.
 * @return  none
 */
void RTC_RTCCLKConfig(uint32_t RTC_RTCCLKSource)
{
    RTC->CTLRL &= ~RTC_RTCCLKSource_HSE_Div512;
    RTC->CTLRL |= RTC_RTCCLKSource;
}
 ...
 *        using the RTC_RTCCLKConfig function.
 *
 * @param   NewState - ENABLE or DISABLE.
 *
 * @return  none
 */
void RTC_RTCCLKCmd(FunctionalState NewState)
{
    if(NewState)
    {
        RTC->CTLRL |= (1 << 6);

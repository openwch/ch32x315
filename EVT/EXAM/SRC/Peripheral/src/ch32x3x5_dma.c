...
/* DMA1 FLAG mask */
#define DMA1_EXTEN_FLAG_Mask      ((uint32_t)0xA0000000)
 ...
 *        1 to 11 for DMA1 to select the DMA Channel.
 *
 * @return  none
 */
void DMA_DeInit(DMA_Channel_TypeDef *DMAy_Channelx)
{
    ...
    {
        DMA1_EXTEN->INTFCR |= DMA1_Channel8_IT_Mask;

    }
    else if(DMAy_Channelx == DMA1_Channel9)
    {
        DMA1_EXTEN->INTFCR |= DMA1_Channel9_IT_Mask;
    }
    else if(DMAy_Channelx == DMA1_Channel10)
    {
        DMA1_EXTEN->INTFCR |= DMA1_Channel10_IT_Mask;
    }
    else if(DMAy_Channelx == DMA1_Channel11)
    {
        DMA1_EXTEN->INTFCR |= DMA1_Channel11_IT_Mask;
    }
}
...
FlagStatus DMA_GetFlagStatus(uint32_t DMAy_FLAG)
{
    ...
    if((DMAy_FLAG & DMA1_EXTEN_FLAG_Mask) == DMA1_EXTEN_FLAG_Mask)
    {
...
ITStatus DMA_GetITStatus(uint32_t DMAy_IT)
{
    ...
    if((DMAy_IT & DMA1_EXTEN_FLAG_Mask) == DMA1_EXTEN_FLAG_Mask)
    {
        tmpreg = DMA1_EXTEN->INTFR;
        tmpreg &= ~(0xFFFFF000);
    }
...
/*********************************************************************
 * @fn      DMA_RemapChannel_Config
 *
 * @brief   Changes the mapping of the specified DMA channel.
 *
 * @param   DMA_Remap - selects the pin to remap.
 *            DMA_RemapCH_SPI1 - SPI1 Alternate Function mapping DMA channel.
 *            DMA_RemapCH_SPI2 - SPI2 Alternate Function mapping DMA channel.
 *            DMA_RemapCH_SPI3 - SPI3 Alternate Function mapping DMA channel. 
 *            DMA_RemapCH_USART1 - USART1 Alternate Function mapping DMA channel.
 *            DMA_RemapCH_USART2 - USART2 Alternate Function mapping DMA channel.
 *            DMA_RemapCH_USART3 - USART3 Alternate Function mapping DMA channel.
 *            DMA_RemapCH_USART4 - USART4 Alternate Function mapping DMA channel.
 *            DMA_RemapCH_I2C2 - I2C2 Alternate Function mapping DMA channel.
...
/*********************************************************************
 * @fn      DMA_GetDoubleBufferMemoryFlag
 *
 * @brief   Get the DMAy Channelx DoubleBuffer memory flag
 *
 * @param   DMAy_Channelx - here y can be 1 to select the DMA and x can be
 *        1 to 11 for DMA1 to select the DMA Channel.
 *  
 * @return  DMA_DoubleBufferMode_Memory_0/1
 */
uint32_t DMA_GetDoubleBufferMemoryFlag(DMA_Channel_TypeDef* DMAy_Channelx)
{
    uint32_t tmpreg = 0;
    tmpreg = ((DMAy_Channelx->CFGR) & 0x10000);
    if(tmpreg == DMA_DoubleBufferMode_Memory_1)
    {
        return DMA_DoubleBufferMode_Memory_1;
    }
    else 
    {
        return DMA_DoubleBufferMode_Memory_0;        
    }

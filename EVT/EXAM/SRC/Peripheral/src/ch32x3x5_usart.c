 ...
 *        reset values.
 *
 * @param   USARTx - where x can be 1 to 4 to select the USART peripheral.
 *
 * @return  none
 */
 ...
 *            USART_FLAG_LPWKUP_ACT - Low Power WakeUp flag.
 ...
 *
 * @param   USARTx - where x can be (1~4) to select the USART peripheral.
...
/*********************************************************************
 * @fn      USART_ForceTXOpenDrainCmd
 *
 * @brief   Enables or disables the USART force TX open drain.
 *
 * @param   USARTx - where x can be (1~4) to select the USART peripheral.
 *          NewState - ENABLE or DISABLE.
 *
 * @return  none
 */
void USART_ForceTXOpenDrainCmd(USART_TypeDef *USARTx, FunctionalState NewState)
{
    if(NewState != DISABLE)
    {
        USARTx->CTLR4 |= USART_CTLR4_SELF_OD;
    }
    else
    {
        USARTx->CTLR4 &= ~USART_CTLR4_SELF_OD;

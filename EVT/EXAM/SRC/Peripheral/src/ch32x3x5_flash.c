 ...
 *          OB_RST - Selects the reset IO pin mode.
 *            OB_RST_NoEN - Disable reset function.
 *            OB_RST_EN - Enable reset function.
 ...
 *          OB_USBHSDL - Selects BOOT USBHS download mode.
 *            OB_USBHSDL_EN - Boot Enable USBHS Download 
 *            OB_USBHSDL_NoEN - Boot Disable USBHS Download 
 *          OB_USARTDL - Selects BOOT USART download mode.
...
FLASH_Status FLASH_UserOptionByteConfig(uint16_t OB_IWDG, uint16_t OB_RST, uint16_t OB_PowerON_Start_Mode,uint16_t OB_USBHSDL,uint16_t OB_USARTDL)
{
    ...
    if(status == FLASH_COMPLETE)
    {
        ...
        /* Write optionbytes */
        pbuf[1] = OB_IWDG | (uint16_t)(OB_RST | (uint16_t)(OB_PowerON_Start_Mode | (uint16_t)(OB_USBHSDL | (uint16_t)(OB_USARTDL | ((uint16_t)temp)))));
 ...
 *            The FLASH User Option Bytes values - IWDG_EN(Bit0)
 *            The FLASH User Option Bytes values - RST_EN(Bit3)
 *            The FLASH User Option Bytes values - START_MD(Bit4)
 *            The FLASH User Option Bytes values - USBHSDL_EN(Bit6)
 ...
 *            FLASH_FLAG_WRBSY - FLASH write Busy flag
 *            FLASH_FLAG_WRPRTERR - FLASH Write protected error flag
 *            FLASH_FLAG_EOP - FLASH End of Operation flag
 *            FLASH_FLAG_BOOT_LOADER - BOOT load flag
 *            FLASH_FLAG_BOOT_AVA - BOOT AVA flag
 *            FLASH_FLAG_BOOT_STATUS - BOOT status flag
...
{
    uint32_t tmp ;
    tmp = FLASH_FLAG & ((uint32_t)(0x0FFFFFFF));
    FlagStatus bitstatus = RESET;

    if((FLASH_FLAG & ((uint32_t)0x40000000)) != 0)
    {
        if((FLASH->ACTLR & tmp) != (uint32_t)RESET)
        {
            bitstatus = SET;
        }
        else
    ...
    {
        if((FLASH->OBR & tmp) != (uint32_t)RESET)
        {
            bitstatus = SET;
        }
        else
    ...
    {
        if((FLASH->STATR & tmp) != (uint32_t)RESET)
        {
            bitstatus = SET;
        }
        else
 ...
 *            FLASH_CLK_HCLKDIV1 - HCLKClock
 *            FLASH_CLK_HCLKDIV2 - HCLKClock/2
 *            FLASH_CLK_HCLKDIV4 - HCLKClock/4
 *            FLASH_CLK_HCLKDIV8 - HCLKClock/8
 *
 * @return  none
 */
void FLASH_Access_Clock_Cfg(uint32_t FLASH_Access_CLK)
{
    uint32_t tmp = FLASH->ACTLR;
    tmp &= ~(3<<0);

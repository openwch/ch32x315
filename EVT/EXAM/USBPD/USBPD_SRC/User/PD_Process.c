...
void PD_Rx_Mode( void )
{
    ...
    USBPD->BMC_CLK_CNT = UPD_TMR_RX_240M;
...
void PD_Init( void )
{
    ...
    RCC_HB2PeriphClockCmd(RCC_HB2Periph_GPIOD, ENABLE);               /* Open PD I/O clock, AFIO clock and PD clock */
    ...
    GPIO_PinAFConfig(GPIOD, GPIO_PinSource4, GPIO_AF2);
    GPIO_PinAFConfig(GPIOD, GPIO_PinSource5, GPIO_AF2);
    AFIO->CR |= USBPDHVT ;
	AFIO->CR &=~(3<<1);
    AFIO->CR |= 2<<1 ; 
...
UINT8 PD_Detect( void )
{
    ...
    if(PD_Ctl.Flag.Bit.Connected)                                       /*Detect disconnection*/
    {
        ...
        if((GPIOD->INDR & PIN_CC1) != (uint32_t)Bit_RESET)
        {
            cmp_cc1 |= bCC_CMP_220;
        }
        if((GPIOD->INDR & PIN_CC2) != (uint32_t)Bit_RESET)
        {
            cmp_cc2 |= bCC_CMP_220;
        }
...
void PD_Phy_SendPack( UINT8 mode, UINT8 *pbuf, UINT8 len, UINT8 sop )
{
    ...
    USBPD->BMC_CLK_CNT = UPD_TMR_TX_240M;

...
void USBSS_Device_Init( FunctionalState sta )                                                     
{
    if( sta )
    {
        ...
        USBSSD->LINK_CFG = LINK_RX_EQ_EN | LINK_TX_DEEMPH_6DB | LINK_PHY_RESET;   
        USBSSD->LINK_CTRL = LINK_P2_MODE | LINK_GO_DISABLED;                        
        USBSSD->LINK_CFG = LINK_RX_EQ_EN | LINK_TX_DEEMPH_6DB | LINK_LTSSM_MODE | LINK_TOUT_MODE ;
 ...
 * @return  none
 */
void USBSS_LINK_Handle( USBSSD_TypeDef *USBSSHx ) 
{
 ...
 * @return  none
 */
void SET_Device_Address( uint32_t address, USBSSD_TypeDef *USBSSx)
{
...
void USBSS_PLL_Init( FunctionalState sta )
{
    if(sta)
    {
        RCC->CTLR |= (uint32_t)RCC_USBSSPLLON;
        /* Wait till USBSS PLL is ready */
        while(( RCC->CTLR & (uint32_t)RCC_USBSSPLLRDY) != (uint32_t)RCC_USBSSPLLRDY );
    }
    else 
    {
        RCC->CTLR &= ~(uint32_t)RCC_USBSSPLLON;

 ...
 *@Note
 slave mode routine:
 TIM1_CH1(PA10)
This example demonstrates that when the TIM_CH1(PA10) pin detects a rising edge,
TIM1 works differently in different slave modes.
 RESET_MODE: TIM1->CNT reset and recount
 GATED_MODE: PA10 pin input low level, TIM1->CNT counts normally, otherwise stop counting.
...
void TIM1_Synchro_ExtTrigger_Init( u16 arr, u16 psc )
{
	...
	RCC_HB2PeriphClockCmd( RCC_HB2Periph_GPIOA|RCC_HB2Periph_AFIO|RCC_HB2Periph_TIM1, ENABLE );
...
int main(void)
{
	...
	TIM1_Synchro_ExtTrigger_Init( 0xFFFF, 24000-1);
	while(1)
	{
		printf("cnt:%d\r\n", TIM1->CNT);
